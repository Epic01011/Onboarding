    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Tâches overdue</h1>
            <p className="text-sm text-gray-500">
              {overdueDossiers.length === 0
                ? 'Aucun dossier en retard'
                : `${overdueDossiers.length} dossier${overdueDossiers.length > 1 ? 's' : ''} sans mise à jour depuis plus de 7 jours`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-6 max-w-4xl mx-auto">
        {overdueDossiers.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">Aucun dossier overdue</p>
            <p className="text-sm text-gray-400 mt-1">Tous vos dossiers sont à jour !</p>
          </div>
        ) : (
          <div className="space-y-3">
            {overdueDossiers.map(dossier => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left bg-white rounded-xl border border-red-200 bg-red-50/30 hover:border-red-400 hover:shadow-md transition-all p-5 group"
                >
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-red-400 group-hover:translate-x-1 transition-all flex-shrink-0" />
                  </div>
                </button>
              );
            })}
